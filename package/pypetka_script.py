...
def объединение_слов(список_слов, разделитель=' '):
   """Объединяет слова в строку с заданным разделителем."""
   return разделитель.join(список_слов)

def разделение_строки(строка, разделитель=' '):
   """Разделяет строку на слова по заданному разделителю."""
   return строка.split(разделитель)

def проверка_вхождения(элемент, список):
   """Проверяет, содержится ли элемент в списке."""
   return элемент in список

def создать_словарь(ключи, значения):
   """Создает словарь из списков ключей и значений."""
   return dict(zip(ключи, значения))

def получить_значение_из_словаря(словарь, ключ, значение_по_умолчанию=None):
   """Возвращает значение по ключу из словаря, или значение по умолчанию."""
   return словарь.get(ключ, значение_по_умолчанию)

def удалить_ключ(словарь, ключ):
   """Удаляет ключ из словаря, если он существует."""
   словарь.pop(ключ, None)
...
def сортировка_словаря(словарь, по_возрастанию=True):
   """Сортирует словарь по ключам."""
   return dict(sorted(словарь.items(), reverse=not по_возрастанию))

def получить_ключи(словарь):
   """Возвращает список ключей из словаря."""
   return list(словарь.keys())

def получить_значения(словарь):
...
def проверить_анAGRAM(строка1, строка2):
...
def получить_пересечение(список1, список2):
   """Возвращает элементы, которые есть в обоих списках."""
   return list(set(список1) & set(список2))
...
def получить_объединение(список1, список2):
   """Возвращает объединение двух списков без дубликатов."""
   return list(set(список1) | set(список2))
...
def получить_среднее_значение_по_значению(список, ключ):
   """Возвращает среднее значение по заданному ключу."""
   значения = [ключ(элемент) for элемент in список]
   return sum(значения) / len(значения) if значения else 0

def проверить_пустоту_списка(список):
   """Проверяет, является ли список пустым."""
   return not список

def получить_первый_элемент_или_значение(список, значение_по_умолчанию=None):
   """Возвращает первый элемент списка или значение по умолчанию."""
   return список[0] if список else значение_по_умолчанию

def получить_последний_элемент_или_значение(список, значение_по_умолчанию=None):
   """Возвращает последний элемент списка или значение по умолчанию."""
   return список[-1] if список else значение_по_умолчанию
...
def получить_объединение_по_значению(список1, список2, ключ):
   """Возвращает объединение двух списков без дубликатов по заданному ключу."""
   ключи = {ключ(элемент) for элемент in список1 + список2}
   return [элемент for элемент in список1 + список2 if ключ(элемент) in ключи]

def найти_дубликаты_по_значению(список, ключ):
   """Возвращает список дубликатов из переданного списка по заданному ключу."""
   seen = set()
   duplicates = set()
   for элемент in список:
...
def получить_первый_дубликат_по_значению(список, ключ):
   """Возвращает первый дубликат из списка по заданному ключу."""
   seen = set()
   for элемент in список:
       ключ_элемента = ключ(элемент)
...
def получить_последний_дубликат_по_значению(список, ключ):
   """Возвращает последний дубликат из списка по заданному ключу."""
   seen = set()
   последний_дубликат = None
   for элемент in список:
...
def получить_пересечение_по_значению(список1, список2, ключ):
   ...
   ключи1 = {ключ(элемент) for элемент in список1}
   return [элемент for элемент in список2 if ключ(элемент) in ключи1]
def генератор_паролей(length=12):
